...
function collectParams(req, res, next) {
  ...
  if (!definition) {
    ...
    throw new Error(JSON.stringify(req.app.get('definitions'), null, 2))
...
function commonSolve(req, res, next) {
  ...
  if (res.locals.cacheResult !== null) {
    ...
    compute.Grasshopper.evaluateDefinition(definitionPath, trees, false).then((response) => {
      ...
      console.log(JSON.stringify(trees, (key, value) => {
        // This replacer function will be called for each key-value pair in the object

        // Handle the expansion of InnerTree
        if (key === 'InnerTree' && typeof value === 'object') {
